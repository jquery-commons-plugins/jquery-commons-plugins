...
(function($, undefined) {
	 ...
	 */
	function splitDateDDMMYYYY(date) {
	...
	$.fn.dataTableExt.oSort['date-ddmmyyyy-asc'] = function(a, b) {
		a = splitDateDDMMYYYY(a);
		b = splitDateDDMMYYYY(b);
	...
	$.fn.dataTableExt.oSort['date-complete-asc'] = function(a, b) {
		a = splitDateComplete(a);
		b = splitDateComplete(b);
		return compareDate(a, b);
	};

	$.fn.dataTableExt.oSort['date-complete-desc'] = function(a, b) {
		a = splitDateComplete(a);
		b = splitDateComplete(b);
		return compareDate(b, a);
	};
	 ...
	 *            additionnal options
	 */
	$.fn.customDatatableFromJSON = function(colsDef, jsonContent, options) {
		var idContainer = $(this).attr('id');
		templateDatatable = $("#" + idContainer);
		...
		var vhtml = '<thead><tr>';
		var i;
		for (i = 0; i < colsDef.lenght; i++) {
			vhtml += '<th></th>';
		}
		vhtml += '</tr></thead><tbody></tbody>';

		var nameDT = "datatable_" + idContainer;

		templateDatatable.html('<table id=' + nameDT + ' name="' + nameDT
				+ '" class="datatable"><tr></tr></table>');
		templateDatatable.find('table[name="' + nameDT + '"]').html(vhtml);
		
		datatable = templateDatatable.find('table[name="' + nameDT + '"]')
				.DataTable({
		...
		$('#' + nameDT + ' tbody').on('click', 'tr', function() {
			$("#" + nameDT + " tbody tr").removeClass('highlighted');
